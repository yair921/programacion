...
const className = 'CtrPelicula';
const collectionName = 'pelicula';
...
class CtrSala {
    ...
    static async add(root, args) {
        ...
        if (exist) {
            return {
                status: false,
                message: `La pelicula ya existe en la base de datos`,
                _id: null
            };
        }
        let resError = {
        ...
        try {
            let newObj = {
                ...args.input,
                idsPeliculaFormato: args.input.idsPeliculaFormato.map(m => ObjectID(m)),
                active: true,
                enabled: true,
                create_at: new Date(),
                updated_at: new Date()
    ...
    static async update(root, input) {
        try {
            if (input.input.idsPeliculaFormato)
                input.input.idsPeliculaFormato = input.input.idsPeliculaFormato.map(m => ObjectID(m));
